...
ES6 문법은 화살표 함수, 템플릿 리터럴, 구조분해 할당을 사용했습니다. GitHub 저장소 배열은 `map`으로 카드 HTML로 바꾸고, 언어 필터와 fork 제외에는 `filter`, 여러 이벤트 연결에는 `forEach`를 사용했습니다.
...
현재 저장소에는 브라우저 캡처 파일이 포함되어 있지 않습니다. 제출 전 최신 Chrome 또는 GitHub Pages 화면에서 세 파일을 캡처해 추가해야 합니다.
